...
from winsecs.utils import winreg, log


class IISCentralCertP:
    ...
    def create_RSAKeyValueFile(self, exe_file, container):
        ...
        try:
            ...
            log.debug('Error executing {container}'.format(container=container))
            tmp_file = ''
    ...
    def run(self):
        ...
        if ccp_enabled != 1:
            log.debug('IIS CentralCertProvider is not enabled')
            return
        ...
        if len(exe_files) == 0:
            ...
            if len(exe_files) == 0:
                log.debug('File not found aspnet_regiis.exe')
                return

        log.info('aspnet_regiis.exe files found: {files}'.format(files=exe_files))
        rsa_xml_file = self.create_RSAKeyValueFile(exe_files[-1], "iisWASKey")
        if rsa_xml_file == '':
            log.debug('Problems extracting RSA Key Value')
            return
        ...
        log.debug('Temporary file removed: {filename}'.format(filename=rsa_xml_file))
...
modules = {"IISCentralCertP": IISCentralCertP()}
