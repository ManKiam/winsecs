...
class KeePassConfig:
    def run(self, profile):
        ...
        # Keepass1
        connection_file_directory = os.path.join(profile['APPDATA'], 'KeePass')
        if os.path.exists(connection_file_directory):
            connection_file_location = os.path.join(connection_file_directory, 'KeePass.ini')
            if os.path.isfile(connection_file_location):
                ...
                if len(file_content.split("KeeKeySourceID")) > 1:
                    ...
                    for i in range(0, KeeKeySource_number ):
                        ...
                        pwd_found.append({'Keyfile': keyfile, 'Database': database})
                # KeeLastDb
                if file_content.partition("KeeLastDb=")[1] == "KeeLastDb=":
                    ...
                    if already_in_pwd_found == 0:
                        pwd_found.append({'Keyfile': "No keyfile found", 'Database': database})
        # Keepass2
        connection_file_directory = os.path.join(profile['APPDATA'], 'KeePass')
        if os.path.exists(connection_file_directory):
            connection_file_location = os.path.join(connection_file_directory, 'KeePass.config.xml')
            ...
            if os.path.isfile(connection_file_location):
                try:
                    ...
                    for connection_node in connection_nodes:
                        ...
                        pwd_found.append({'Keyfile': keyfile, 'Database': database, 'Type': type[:-3]})
                except:
                    pass
...
modules = {"KeePassConfig": KeePassConfig()}
