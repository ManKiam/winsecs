...
class WinSCP:
    ...
    def check_winscp_installed(self):
        try:
            key = OpenKey(winreg.HKEY_CURRENT_USER, 'Software\\Martin Prikryl\\WinSCP 2\\Configuration\\Security')
            return key
    ...
    def get_credentials(self):
        try:
            key = OpenKey(winreg.HKEY_CURRENT_USER, 'Software\\Martin Prikryl\\WinSCP 2\\Sessions')
        except Exception as e:
            log.debug(str(e))
            return False
    ...
    def run(self):
        ...
        if winscp_key:
            if not self.check_masterPassword(winscp_key):
                ...
                log.warning('A master password is used. Passwords cannot been retrieved')


modules = {"WinSCP": WinSCP()}
